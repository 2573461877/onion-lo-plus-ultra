...
private:
    ...
    std::string motion_frame_ = "motion_link";
    std::string odometry_topic_ = "/fast_lio_humanoid/odometry";
    std::string scan_topic_ = "/fast_lio_humanoid/cloud_registered";
    std::string initialpose_topic_ = "/initialpose";
    std::string localization_topic_ = "/fast_lio_humanoid/localization";
    std::string confidence_topic_ =
        "/fast_lio_humanoid/localization_confidence";
    std::string delay_topic_ = "/fast_lio_humanoid/localization_delay_ms";
    std::string save_path_ = "/tmp/fast_lio_localization_humanoid/scans/";
};
                                                                                             ...
                                                                                             nh_private_(nh_private)
{
    ...
    nh_private_.param<std::string>("frames/map", map_frame_, map_frame_);
    nh_private_.param<std::string>("frames/odom", odom_frame_, odom_frame_);
    nh_private_.param<std::string>("frames/body", body_frame_, body_frame_);
    nh_private_.param<std::string>("frames/base", base_frame_, base_frame_);
    nh_private_.param<std::string>("frames/imu", imu_frame_, imu_frame_);
    nh_private_.param<std::string>(
        "frames/motion", motion_frame_, motion_frame_);
    nh_private_.param<std::string>(
        "topics/odometry", odometry_topic_, odometry_topic_);
    nh_private_.param<std::string>("topics/scan", scan_topic_, scan_topic_);
    nh_private_.param<std::string>(
        "topics/initialpose", initialpose_topic_, initialpose_topic_);
    nh_private_.param<std::string>(
        "topics/localization", localization_topic_, localization_topic_);
    nh_private_.param<std::string>(
        "topics/confidence", confidence_topic_, confidence_topic_);
    nh_private_.param<std::string>("topics/delay", delay_topic_, delay_topic_);
    nh_private_.param<std::string>("save_path", save_path_, save_path_);

    int publisher_queue_size = 10;
    ...
    nh_private_.param<int>(
        ...
        subscriber_queue_size,
        subscriber_queue_size);
    publisher_queue_size = std::max(1, publisher_queue_size);
    subscriber_queue_size = std::max(1, subscriber_queue_size);

    pub_baselink2map_ = nh.advertise<nav_msgs::Odometry>(
        "/fast_lio_humanoid/baselink2map", publisher_queue_size);
    pub_baselink2map_kalman_ = nh.advertise<nav_msgs::Odometry>(
        "/fast_lio_humanoid/baselink2map_kalman", publisher_queue_size);
    pub_motionlink2map_ = nh.advertise<nav_msgs::Odometry>(
        "/fast_lio_humanoid/motionlink2map", publisher_queue_size);
    pub_odom2map_ = nh.advertise<nav_msgs::Odometry>(
        "/fast_lio_humanoid/odom2map", publisher_queue_size);
    pub_odom2map_kalman_ = nh.advertise<nav_msgs::Odometry>(
        "/fast_lio_humanoid/odom2map_kalman", publisher_queue_size);

    pub_map_ = nh.advertise<sensor_msgs::PointCloud2>(
        "/fast_lio_humanoid/localization_map", 1, true);
    pub_submap_ = nh.advertise<sensor_msgs::PointCloud2>(
        "/fast_lio_humanoid/submap", 1, true);
    pub_scan2map_ = nh.advertise<sensor_msgs::PointCloud2>(
        "/fast_lio_humanoid/scan2map", 1, true);
    pub_scan_ = nh.advertise<sensor_msgs::PointCloud2>(
        "/fast_lio_humanoid/scan", 1, true);
    pub_localization_3d_ = nh.advertise<geometry_msgs::PoseStamped>(
        localization_topic_, 1, false);
    pub_localization_3d_confidence_ = nh.advertise<std_msgs::Float32>(
        confidence_topic_, 1, false);
    pub_localization_3d_delay_ms_ = nh.advertise<std_msgs::Float32>(
        delay_topic_, 1, false);

    loc_frequence_ = 2.0; //
    loc_fitness_ = 0.0;
    // 注册回调函数
    sub_baselink2odom_ = nh_.subscribe(
        odometry_topic_,
        subscriber_queue_size,
        &GloabalLocalization::CallbackBaselink2Odom,
        this);
    sub_scan_cur_ = nh_.subscribe(
        scan_topic_,
        subscriber_queue_size,
        &GloabalLocalization::CallbackScan,
        this);
    sub_initialpose_ = nh_.subscribe(
        initialpose_topic_,
        subscriber_queue_size,
        &GloabalLocalization::CallbackInitialPose,
        this);
    ...
    if (save_scan_ &&
        !boost::filesystem::exists(save_path_) &&
        !boost::filesystem::create_directories(save_path_))
    {
        ROS_ERROR_STREAM("Unable to create localization scan directory: "
                         << save_path_);
        save_scan_ = false;
    }
    /// 最大点数量限制
    ...
    nh_private_.param<std::vector<float>>(
        "initialpose", initialpose_, std::vector<float>(6, 0.0f));
    nh_private_.param<double>("dis_updatemap", dis_updatemap_, 5);

    if (initialpose_.size() != 6)
    {
        ROS_FATAL("initialpose must contain x y z roll pitch yaw");
        ros::shutdown();
        return;
    }
    ...
    if (!map_loaded || pcd_map_ori_ == nullptr || pcd_map_ori_->IsEmpty())
    {
    ...
    if (!GetTfTransformToMatrix(
            base_frame_, imu_frame_, mat_imulink2baselink_))
    {
        ROS_WARN("Using identity base-to-IMU transform");
    }
    std::cout << "mat_imulink2baselink_:\n"
              << mat_imulink2baselink_ << std::endl;

    if (!GetTfTransformToMatrix(
            motion_frame_, base_frame_, mat_baselink2motionlink_))
    {
        ROS_WARN("Using identity motion-to-base transform");
    }
...
{
    {
        std::lock_guard<std::mutex> lock(lock_exit_);
        flag_exit_ = true;
    }
    ...
    const nav_msgs::Odometry::ConstPtr &baselink2odom)
{
    ...
    br_odom2map.sendTransform(
        tf::StampedTransform(
            transform_odom2map,
            baselink2odom->header.stamp,
            map_frame_,
            odom_frame_));
...
{
    ROS_INFO_STREAM("wait for odometry: " << odometry_topic_);
    if (!ros::topic::waitForMessage<nav_msgs::Odometry>(
            odometry_topic_, nh_, ros::Duration(60.0)))
    {
        ROS_ERROR_STREAM("Timed out waiting for " << odometry_topic_);
        return;
    }
    ROS_INFO_STREAM("wait for registered scan: " << scan_topic_);
    if (!ros::topic::waitForMessage<sensor_msgs::PointCloud2>(
            scan_topic_, nh_, ros::Duration(60.0)))
    {
        ROS_ERROR_STREAM("Timed out waiting for " << scan_topic_);
        return;
    }
    // initialize
    /****初始化定位****/
        ...
        else
        {
            ...
            *target = *map_fine_crop;
            ...
            if (save_scan_)
            {
                ...
                const boost::filesystem::path scan_directory(save_path_);
                open3d::io::WritePointCloud(
                    (scan_directory /
                     (std::to_string(scan_count) + "_ori.ply")).string(),
                    *pcd_scan);
                open3d::io::WritePointCloud(
                    (scan_directory /
                     (std::to_string(scan_count) + "_crop.ply")).string(),
                    *pcd_scan2map);
                scan_count += 1;
            }
